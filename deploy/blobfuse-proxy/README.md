...
helm install blob-csi-driver blob-csi-driver/blob-csi-driver --namespace kube-system --version v1.23.11 --set node.enableBlobfuseProxy=true
```
...
> blobfuse proxy is supported on CoreOS(OpenShift) from v1.23.11
```console
kubectl apply -f https://raw.githubusercontent.com/kubernetes-sigs/blob-csi-driver/master/deploy/v1.23.11/blobfuse-proxy.yaml
```
