...
allowSharedKeyAccess | Allow or disallow shared key access for storage account created by driver. Applies to both blobfuse and NFS accounts. <br><br>**NFS:** safe to disable — NFS mount does not use the account key. <br><br>**blobfuse/blobfuse2:** disabling shared key access is supported when combined with identity-based auth — set `AzureStorageAuthType` to `MSI` or `SPN`, or use `mountWithWorkloadIdentityToken=true`; the driver then automatically sets `storeAccountKey=false`. If none of those identity-based auth options are set, the default `storeAccountKey=true` conflicts with `allowSharedKeyAccess=false` and the controller will reject the request. | `true`,`false` | No | `true`
...
volumeAttributes.mountWithWorkloadIdentityToken (Preview) | indicates whether performing blobfuse mount with workload identity token | `true`,`false` (use the type "string" instead of "bool") | No | `false`
