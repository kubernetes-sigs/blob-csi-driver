# Install Azure Blob Storage CSI driver v1.23.1 version on a kubernetes cluster
...
cd blob-csi-driver
./deploy/install-driver.sh v1.23.1 local,blobfuse-proxy
```
...
```console
curl -skSL https://raw.githubusercontent.com/kubernetes-sigs/blob-csi-driver/v1.23.1/deploy/uninstall-driver.sh | bash -s v1.23.1 --
```
...
./deploy/uninstall-driver.sh v1.23.1 local
```
