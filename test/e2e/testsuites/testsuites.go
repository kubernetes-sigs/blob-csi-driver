...
func (t *TestDeployment) Cleanup(ctx context.Context) {
	...
	body, err := t.Logs(ctx)
	if err != nil {
		framework.Logf("Error getting logs for pod %s: %w", t.podName, err)
	} else {
...
func cleanupPodOrFail(ctx context.Context, client clientset.Interface, name, namespace string) {
	...
	if err != nil {
		framework.Logf("Error getting logs for pod %s: %w", name, err)
	} else {
...
func waitForPersistentVolumeClaimDeleted(ctx context.Context, c clientset.Interface, pvcName string, ns string, Poll, timeout time.Duration) error {
