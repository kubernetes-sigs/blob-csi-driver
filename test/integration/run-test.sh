...
if [ -v aadClientSecret ]; then
	...
	value=`$csc controller new --endpoint $endpoint --cap 1,block CSIVolumeName --req-bytes 2147483648 --params skuname=Standard_LRS,resourceGroup=$resourceGroup`
	retcode=$?
	...
	echo "create volume in a specified storage account:"
