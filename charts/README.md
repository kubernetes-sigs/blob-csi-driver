...
helm install blob-csi-driver blob-csi-driver/blob-csi-driver --set node.enableBlobfuseProxy=true --namespace kube-system --version 1.27.3
```
...
| `image.csiResizer.tag`                                | csi-resizer docker image tag                          | `v2.0.0`                                                         |
...
| `node.hostPaths.usr` | host path for the `host-usr` volume; also used to set the init container mountPath to `/host{path}` | `/usr`
| `node.hostPaths.usrLocal` | host path for the `host-usr-local` volume; also used to set the init container mountPath to `/host{path}` | `/usr/local`
| `node.hostPaths.etc` | host path for the `host-etc` volume; also used to set the init container mountPath to `/host{path}` | `/etc`
