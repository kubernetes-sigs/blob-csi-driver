...
func (d *Driver) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	// shouldCleanupContainer indicates whether the container should be cleaned up on failure.
	// When containerName is user-specified (non-empty), we skip cleanup since the container
...
// isAzcopyJobActiveOrCompleted reports whether an azcopy job for a container is either
// still in flight or has already reached a terminal completion state (with or without
// per-file errors/skips). Any of these means the container is meaningful and must not
// be deleted by the CreateVolume cleanup helper: an in-flight job would be orphaned,
// and a completed-with-errors/skipped job has already produced usable data that the
// caller may retry to reconcile.
func isAzcopyJobActiveOrCompleted(state util.AzcopyJobState) bool {
	switch state {
	case util.AzcopyJobRunning,
		util.AzcopyJobCompleted,
		util.AzcopyJobCompletedWithErrors,
		util.AzcopyJobCompletedWithSkipped,
		util.AzcopyJobCompletedWithErrorsAndSkipped:
		return true
	}
	return false
}

// cleanupContainerOnFailure is a best-effort rollback helper that deletes the blob container
// when shouldCleanupContainer is true (i.e., the container name was auto-generated by CSI).
...
func (d *Driver) cleanupContainerOnFailure(shouldCleanupContainer bool, subsID, resourceGroup, accountName, containerName string, secrets map[string]string, authAzcopyEnv []string, reason string) {
	...
	klog.V(2).Infof("%s on account(%s), cleaning up container(%s)", reason, accountName, containerName)
