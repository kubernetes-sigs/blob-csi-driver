...
var (
	...
	operationDurationWithLabels = metrics.NewHistogramVec(
		...
		[]string{"operation", "success", "volume_id", "protocol", "storage_account_type", "account_name", "container_name", "is_hns_enabled"},
	)
...
func (mc *CSIMetricContext) Observe(success bool) {
	...
	if len(mc.labels) > 0 {
		volumeID := mc.labels["volume_id"]
		protocol := mc.labels["protocol"]
		storageAccountType := mc.labels["storage_account_type"]
		accountName := mc.labels["account_name"]
		containerName := mc.labels["container_name"]
